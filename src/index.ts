...
const module: TextlintRuleModule = (context, userOptions) => {
    ...
    const allowedKanjis = [
      ...options.allowN5Kanjis ? n5 : [],
      ...options.allowN4Kanjis ? n4 : [],
      ...options.allowN3Kanjis ? n3 : [],
      ...options.allowN2Kanjis ? n2 : [],
      ...options.allowN1Kanjis ? n1 : [],
    ];
    ...
    const blockedKanjiMessage =
      allowedGrades.length === 0 ? '漢字'
        : allowedGrades.length === 5 ? 'JLPT ではない漢字'
          : `${allowedGrades.join('、')} で学習しない漢字`;
    ...
    return {
      async [Syntax.Str](node) {
        ...
        while (word = regex.exec(text)) {
          if (allowedKanjis.includes(word[0])) {
            continue;
          }
  
          const ruleError = new RuleError(
            `${blockedKanjiMessage}「${word[0]}」が含まれています。`,
            { index: word.index }
          );
